/*
  ...
  Device -> Software:
    ...
    STETH_DONE:1
    VITALS:<bpm>,<spo2>
    PULSE_WAITING:1         no finger detected yet
    PULSE_ACTIVE:1
    PULSE_DONE:1
    PULSE_CANCELLED:1
    TEMP:<celsius>
...
void reportMenuHighlight() {
  ...
  Serial.println(activeMenu()[menuIndex].navToken);
...
// Plausible forehead-IR window. Readings outside it mean the sensor is aimed
// at the room, not a person, so they must not be presented as a body
// temperature on a device whose whole purpose is triage.
const float TEMP_MIN_VALID_C = 30.0;
...
void setup() {
  ...
  if (!tempSensorOk) Serial.println("ERR:SENSOR");
...
bool readBodyTemp(float &out) {
  ...
  if (isnan(c) || c < TEMP_MIN_VALID_C || c > TEMP_MAX_VALID_C) return false;
  out = c;
  return true;
}
...
void handleCommand(const String &line) {
  ...
  if (line == "PING") {
    ...
    Serial.print(",");
    Serial.print(stethSub);
    Serial.print(",");
    Serial.print(vitalsDone ? 1 : 0);
    Serial.print(",");
    Serial.print(tempDone ? 1 : 0);
    Serial.print(",");
    Serial.println(xrayUploaded ? 1 : 0);
  ...
  // ─── Measurements ───────────────────────────────────────────────
    ...
    Serial.println("PULSE_WAITING:1");
    ...
    Serial.println("PULSE_CANCELLED:1");
    ...
    Serial.println("TEMP_ACTIVE:1");

  } else if (line == "STOP_TEMP") {
    if (tempSub == SUB_ACTIVE) {
      tempSub = tempValid ? SUB_DONE : SUB_IDLE;
      tempDone = tempValid;
      digitalWrite(PIN_TEMPLED, LOW);
      if (tempValid) {
        Serial.print("TEMP:");
        Serial.println(tempValue, 2);
        Serial.println("TEMP_DONE:1");
      } else {
  ...
  // ─── Session / identity ─────────────────────────────────────────
    ...
    staffName = "";
    patientName = "";
    Serial.println("MODE_ACK:GUEST");

  } else if (line == "MODE:STAFF") {
    setMode(MODE_STAFF);
    Serial.println("MODE_ACK:STAFF");

  } else if (line.startsWith("STAFF:")) {
    staffName = argOf(line);
    Serial.print("STAFF_ACK:");
    Serial.println(staffName);

  } else if (line.startsWith("PATIENT:")) {
    patientName = argOf(line);
    Serial.print("PATIENT_ACK:");
...
void handleMenu(bool up, bool down, bool select, bool back) {
  ...
  if (select) {
    ...
    Serial.println(item.navToken);
    Serial.print("NAV:");
    Serial.println(item.navToken);
    goToState(item.state);
  }
  if (back) {
    Serial.println("NAV:HOME");
    goToState(STATE_INTRO);
  }
}
...
void handleXray(bool select, bool back) {
  if (back) { Serial.println("NAV:HOME"); goToState(STATE_INTRO); return; }

  if (select && xraySub == SUB_DONE) {
    xrayUploaded = false;
    xraySub = SUB_WAITING;
    Serial.println("XRAY_STATUS:0");
  }
}
...
void handlePulse(bool select, bool back) {
  ...
  if (select) {
    if (pulseSub == SUB_IDLE || pulseSub == SUB_DONE) {
      ...
      // STOP mid-reading -> finalize with best-known values, immediately
      pulseSub = SUB_DONE;
      pulseBufferFillIndex = 0;
      vitalsDone = true;
      digitalWrite(PIN_PULSELED, LOW);
...
void runPulseMeasurement() {
  ...
  if (pulseBufferFillIndex == 0) {
    ...
    if (quickIR < 50000) {
      if (pulseSub != SUB_WAITING) Serial.println("PULSE_WAITING:1");
      pulseSub = SUB_WAITING;
      return;
    }
    if (pulseSub != SUB_ACTIVE) Serial.println("PULSE_ACTIVE:1");
    pulseSub = SUB_ACTIVE;
  }
  ...
  if (validHeartRate) bpmValue = heartRateRaw;
  if (validSPO2)      spo2Value = spo2Raw;

  // Publish every completed sensor window so the kiosk updates live without
...
void handleTemp(bool select, bool back) {
  ...
  if (select) {
    if (tempSub == SUB_IDLE || tempSub == SUB_DONE) {
      ...
      // STOP -> lock the reading in, but only if it is a plausible body
      // temperature. Locking in a 22 C room reading and labelling it a
      // patient temperature is worse than reporting nothing.
      digitalWrite(PIN_TEMPLED, LOW);
      if (tempValid) {
        tempSub = SUB_DONE;
        tempDone = true;
...
// ---------------------------------------------------------------------------
// ANIMATED FINGER GUIDE (bottom-to-top approach to sensor)
// ---------------------------------------------------------------------------
void drawAnimatedFingerGuide() {
  // Update animation frame every 80ms
  ...
  // Sensor circle at center-top area
  int sensorX = 64;
  int sensorY = 22;
  int sensorR = 12;

  // Pulsing rings around sensor
  for (int i = 0; i < 3; i++) {
    int ringPhase = (animFrame + i * 13) % 40;
    int ringR = sensorR + (ringPhase * 20 / 40);
    if (ringR > 0 && ringR < 30) {
      display.drawCircle(sensorX, sensorY, ringR, SSD1306_WHITE);
    }
  }

  // Sensor circle (filled)
  display.fillCircle(sensorX, sensorY, sensorR, SSD1306_WHITE);
  display.fillCircle(sensorX, sensorY, sensorR - 3, SSD1306_BLACK);

  // Small LED dot on sensor
  display.fillCircle(sensorX, sensorY, 2, SSD1306_WHITE);

  // Animated finger moving from bottom to sensor
  // 40 frames: frames 0-30 = moving up, frames 31-39 = pause near sensor
  int fingerY;
  if (animFrame < 30) {
    // Moving phase: bottom (58) to near sensor (34)
    fingerY = 58 - (animFrame * 24 / 30);
  } else {
    // Pause phase: hold near sensor
    fingerY = 34 + ((animFrame - 30) * 2 / 9);
  }
  int fingerX = 64;

  // Finger body (rectangle)
  display.fillRect(fingerX - 6, fingerY - 12, 12, 20, SSD1306_WHITE);
  // Fingertip (rounded top)
  display.fillCircle(fingerX, fingerY - 12, 6, SSD1306_WHITE);
  // Nail highlight
  display.fillRect(fingerX - 3, fingerY - 16, 6, 4, SSD1306_BLACK);
...
// ---------------------------------------------------------------------------
// ANIMATED TEMPERATURE GUIDE (thermometer approach to forehead)
// ---------------------------------------------------------------------------
void drawAnimatedTempGuide() {
  ...
  // Forehead silhouette (arc at top)
  int headX = 64;
  int headY = 20;
  display.drawCircle(headX, headY + 30, 25, SSD1306_WHITE);
  // Fill only the top arc portion
  for (int y = headY + 5; y < headY + 30; y++) {
    int dx = 25 - (y - headY - 5);
    if (dx > 0) {
      display.drawPixel(headX - dx, y, SSD1306_WHITE);
      display.drawPixel(headX + dx, y, SSD1306_WHITE);
    }
  }

  // Animated thermometer moving from right to forehead
  int thermoX;
  if (animFrame < 20) {
    // Moving phase: right (110) to forehead (64)
    thermoX = 110 - (animFrame * 46 / 20);
  } else {
    // Pause phase: hold at forehead
    thermoX = 64 + ((animFrame - 20) * 2 / 10);
  }
  int thermoY = 18;

  // Thermometer body
  display.fillRect(thermoX - 2, thermoY - 10, 4, 20, SSD1306_WHITE);
  // Thermometer bulb
  display.fillCircle(thermoX, thermoY + 12, 4, SSD1306_WHITE);
  // Mercury line
  display.fillRect(thermoX - 1, thermoY - 8, 2, 16, SSD1306_BLACK);
  display.fillRect(thermoX - 1, thermoY + 4, 2, 8, SSD1306_WHITE);

  // Live reading, not a placeholder. This used to print a hardcoded "36.5",
  // which on a thermometer screen reads as a measured value.
  display.setTextSize(1);
  display.setCursor(thermoX + 8, thermoY - 5);
  if (tempValid) {
  ...
  // Aim prompt, so an out-of-range reading explains itself instead of just
  // showing dashes.
  if (!tempValid) {
    drawCentered("AIM AT FOREHEAD", 0, 44, 128, 10, 1);
  }
